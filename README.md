...
- **Bracket**: build a custom knockout bracket. Add rounds (e.g.
  ...
  currently holds that spot, so there is no manual re-seeding once group
  results change. The winner is highlighted automatically; if scores are
  tied you can pick the winner manually (e.g. penalties).
...
- **Backup / Restore**: since data lives only on this one device, the
  ...
  and "Restore Backup" (loads one back in). Use it to keep a copy or move
  the tournament to another phone.
...
- Data is stored per-browser via `localStorage`. It does not sync between
  devices; it's local to whichever phone or browser you use it on. Use
